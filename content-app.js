(() => {
  ...
  const renderRefundPolicyModal = () => {
    const existing = document.getElementById('refund-policy-modal');
    if (existing) return existing;

    const modal = document.createElement('div');
    modal.id = 'refund-policy-modal';
    modal.className = 'policy-modal hidden';
    modal.setAttribute('role', 'dialog');
    modal.setAttribute('aria-modal', 'true');
    modal.setAttribute('aria-labelledby', 'refund-policy-title');
    modal.innerHTML = `
      <div class="policy-backdrop" data-policy-close></div>
      <div class="policy-panel" role="document">
        <button type="button" class="policy-close" data-policy-close aria-label="Close refund policy">
          <iconify-icon icon="solar:close-circle-linear"></iconify-icon>
        </button>
        <div class="flex items-center gap-2 mb-3">
          <span class="w-2 h-2 bg-amber-400 rounded-full"></span>
          <span class="text-xs font-mono uppercase tracking-[0.2em] text-amber-400">Commercial Terms &amp; Policies</span>
        </div>
        <h2 id="refund-policy-title" class="text-2xl sm:text-4xl font-extrabold text-white font-display tracking-tight">Refund &amp; Cancellation Policy</h2>
        <p class="text-xs font-mono uppercase tracking-widest text-neutral-500 mt-2">Effective: 2026 · Olympus Atelier</p>
        
        <div class="mt-8 space-y-6 text-sm text-neutral-300 leading-relaxed border-t border-white/10 pt-6">
          <section class="space-y-2">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">01.</span> Date Reservation &amp; Commission Retainers
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              When booking Olympus Atelier for full-day wedding videography, multi-camera event coverage, or commercial shoots, a commitment deposit (typically 30% to 50% of the total commission) is required to secure your date exclusively on our production calendar. Because we decline all other conflicting enquiries once booked, this retainer is non-refundable and covers preliminary creative scheduling, date reservation, and crew allocation.
            </p>
          </section>

          <section class="space-y-2">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">02.</span> Standalone Video Editing &amp; Post-Production
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              For dedicated video editing, color grading, motion graphics, and post-production commissions:
            </p>
            <ul class="list-disc list-inside space-y-1.5 text-xs sm:text-sm text-neutral-400 pl-2">
              <li><strong>Prior to project commencement:</strong> If a project is cancelled in writing before rushes ingest or editing starts, 100% of prepayments (minus any nominal transaction processing fees) will be refunded.</li>
              <li><strong>In-progress commissions:</strong> Once initial cuts, timeline sequencing, or color passes have been delivered, milestone fees are non-refundable. All project files, exports, and assets produced up to the termination point will be delivered to the client.</li>
            </ul>
          </section>

          <section class="space-y-2">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">03.</span> Rescheduling &amp; Date Adjustments
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              We understand schedules can shift. If you need to reschedule an event or wedding date with at least <strong>14 days' written notice</strong>, 100% of your deposit will be applied directly to the new date (subject to atelier calendar availability within 12 months). Cancellations made with less than 14 days' notice are subject to deposit forfeiture to compensate for lost booking opportunities and crew commitments.
            </p>
          </section>

          <section class="space-y-2">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">04.</span> Quality Guarantee &amp; Revisions
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              All production contracts include dedicated review rounds (as specified in your package or project brief). We work closely with you through audio mixing, rhythm pacing, and color grading adjustments to ensure the final master exceeds your expectations.
            </p>
          </section>

          <section class="space-y-2">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">05.</span> Force Majeure &amp; Emergency Coverage
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              In the unlikely event that severe weather emergencies, natural disasters, or unforeseen force majeure events prevent coverage, we provide priority rescheduling or issue a full production credit voucher valid for any future atelier service.
            </p>
          </section>

          <section class="space-y-2 border-t border-white/10 pt-4">
            <h3 class="text-base font-bold text-white font-display flex items-center gap-2">
              <span class="text-amber-400 font-mono text-xs">06.</span> Direct Resolution &amp; Enquiries
            </h3>
            <p class="text-neutral-400 text-xs sm:text-sm">
              Have questions regarding an invoice, adjustment, or cancellation? Message our production desk directly for immediate assistance:
            </p>
            <div class="flex flex-wrap gap-3 pt-2">
              <a href="https://wa.me/2348087172313?text=Hello%20Olympus%20Atelier,%20I%20have%20an%20enquiry%20regarding%20billing%20and%20cancellation%20policy." target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 rounded-full bg-gold-gradient text-neutral-950 px-5 py-2.5 text-xs font-bold uppercase tracking-wider hover:scale-105 transition-transform">
                <iconify-icon icon="logos:whatsapp-icon"></iconify-icon>
                <span>Chat with Atelier on WhatsApp</span>
              </a>
              <button type="button" class="inline-flex items-center gap-2 rounded-full border border-white/20 text-white hover:border-amber-400 hover:text-amber-400 px-5 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors" data-policy-close>
    ...
    const close = () => {
      modal.classList.add('hidden');
      document.documentElement.style.overflow = '';
      if (window.location.hash === '#refund-policy' || window.location.hash === '#refund') {
    ...
    const open = () => {
      ...
      modal.classList.remove('hidden');
      document.documentElement.style.overflow = 'hidden';
      modal.querySelector('.policy-close')?.focus();
    };
    ...
    modal.openPolicy = open;
    modal.closePolicy = close;
    return modal;
  };
  ...
  const initializeManagedContent = () => {
    ...
    const policyModal = renderRefundPolicyModal();
    window.openOlympusRefundPolicy = () => policyModal.openPolicy();
    document.querySelectorAll('[data-open-refund-modal]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        policyModal.openPolicy();
      });
    });
